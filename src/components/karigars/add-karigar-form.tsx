...
import { KARIGAR_SPECIALIZATIONS, KARIGAR_SPECIALIZATION_LABELS } from "@/types/karigars";
import { createKarigarAction } from "@/lib/actions/karigars.actions";
...
const initialState = {
  ...
  address: "",
  city: "",
  specialization: "OTHER" as (typeof KARIGAR_SPECIALIZATIONS)[number],
  notes: "",
};
...
export function AddKarigarForm() {
  ...
  function handleSubmit() {
    startTransition(async () => {
      const result = await createKarigarAction({
        name: fields.name,
        phone: fields.phone,
      ...
      router.push(`/karigars/${result.data.id}`);
    });
  }
    ...
    <div className="grid gap-6 lg:grid-cols-3">
      ...
      <div className="flex flex-col gap-6">
        <Card>
          ...
          <CardContent>
            <Select value={fields.specialization} onValueChange={(v) => set("specialization", v as typeof fields.specialization)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
