...
export const metadata = { title: "Supplier Ledger | Zarghoon Jewellers" };
...
export default async function SupplierLedgerPage() {
  ...
  const [gold, cash] = await Promise.all([
    listAllGoldLedgerEntries({ partyType: "SUPPLIER", pageSize: 50 }),
    listAllPartyCashLedgerEntries({ partyType: "SUPPLIER", pageSize: 50 }),
  ]);

  return (
    <div className="flex flex-1 flex-col gap-6 p-4 sm:p-6">
      <div>
        <h1 className="font-display text-xl font-semibold text-foreground">Supplier Ledger</h1>
        <p className="text-sm text-muted-foreground">
          Every gold and cash transaction across every supplier — combined for reference, gold and cash are never
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {cash.rows.length === 0 ? (
            <p className="px-4 py-10 text-center text-sm text-muted-foreground">No supplier cash transactions yet.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Credit (Payment)</TableHead>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {gold.rows.length === 0 ? (
            <p className="px-4 py-10 text-center text-sm text-muted-foreground">No supplier gold transactions yet.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
