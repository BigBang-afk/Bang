 ...
 * route protection. Fine-grained authorization (e.g. admin-only) still
 * happens again in each protected layout — this is defense in depth, not
 * the only gate.
 */
export async function updateSession(request: NextRequest) {
  ...
  // Supabase docs: any code here can invalidate the session refresh.
  const {
    data: { user },
