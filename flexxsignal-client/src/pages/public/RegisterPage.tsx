...
export default function RegisterPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      setSession(data);
      pushToast("Account created. Welcome to FlexX Signal!", "success");
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-16">
      <div className="glass-card w-full max-w-md p-8">
        ...
        <p className="text-sm text-slate-400 mb-6">Start with the Free plan — no card required.</p>

        <form onSubmit={handleSubmit((v) => mutation.mutate(v))} className="space-y-4">
          <div>
            <label htmlFor="displayName" className="label-text">Full name</label>
            <input id="displayName" className="input-field" {...register("displayName")} />
            {errors.displayName && <p className="text-signal-down text-xs mt-1">{errors.displayName.message}</p>}
          </div>
          <div>
          ...
          <div>
            <label htmlFor="preferredLanguage" className="label-text">Preferred analysis language</label>
            <select id="preferredLanguage" className="input-field" {...register("preferredLanguage")}>
